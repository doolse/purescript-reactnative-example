...
moviesApp :: ReactClass Unit
moviesApp = createClass $ spec unit render
  where
    render this = pure $ navigator' _ {ref = refFunc $ mkEffFn1 addBackListener, style = sheet.container, configureScene = sceneConfig sceneConfigs.fadeAndroid } initialRoute (sceneRenderer routeMapper)
    addBackListener navU = case toMaybe navU of
        ...
        _ -> pure unit
    initialRoute = Search
    routeMapper Search nav = searchScreen nav
    routeMapper (ShowMovie m) nav = view (styles [flex 1]) [
...
main = registerComponent "reactnativeMovieExample" moviesApp
